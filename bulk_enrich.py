...
for filepath, strategy in csv_files:
    try:
...
# Find all instances of epsGrowth: '—' and revGrowth: '—' with their tickers
pattern = r"ticker: '([^']+)', country: '([^']*)', currency: '([^']*)', epsGrowth: '—', revGrowth: '—'"

matches = list(re.finditer(pattern, content))
print(f"Found {len(matches)} cards with '—' growth values\n")
...
for match in reversed(matches):  # Reverse to maintain positions
    ...
    if ticker in ticker_data:
        ...
        new_text = f"ticker: '{ticker}', country: '{country}', currency: '{currency}', epsGrowth: '{eps_val}', revGrowth: '{rev_val}'"
