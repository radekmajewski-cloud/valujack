...
# Target JOKER cards 236-247 (the ones with old metrics)
updated = 0

for card_id in range(236, 248):
    # Find card with old metric format
    pattern = r"({ id: " + str(card_id) + r", type: 'JOKER'.*?ticker: '([^']+)'.*?)(metrics: \[{ l: 'P/E'.*?\]\])"
    ...
    if match:
        ...
        if ticker in joker_data:
            ...
            updated += 1
            print(f"  Card {card_id} ({ticker}): Updated to JOKER format")
