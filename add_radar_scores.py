...
# Strategy-specific radar dimensions
RADAR_MAP = {
...
# Find all cards with empty scores array
...
def add_scores(match):
    ...
    for label in RADAR_MAP[strategy]:
        ...
        if comp_key and comp_key in components:
            value = int(components[comp_key] / 10)  # Convert 0-100 to 0-10
    ...
    if scores_list:
        ...
        updated += 1
        if updated <= 5:
            print(f"Updated {ticker} ({strategy})")
